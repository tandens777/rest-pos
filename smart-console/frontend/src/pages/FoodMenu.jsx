...
const FoodMenu = () => {
    ...
    const handleCategoryClick = async (id, parentCatId) => {
        try {
            ...
            const response = await axiosInstance.get(`/item/all_subitems/${id || 0}`);
    ...
    const fetchDropdownData = async () => {
        try {
            const [unitsResponse, stationResponse, categoriesResponse] = await Promise.all([
                ...
                axiosInstance.get("/item/getcategories"),
            ]);
    ...
    const handleSearch = async () => {
        try {
            const response = await axiosInstance.get(`/item/all?search=${searchTerm}`);
    ...
    const handleModalSubmit = async (values) => {
        try {
            ...
            const url = editingFoodMenu ? `/item/update/${editingFoodMenu.id}` : `/item/add`;
