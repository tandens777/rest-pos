...
public interface SurDiscRepository extends JpaRepository<SurchargeDiscount, Integer> {
    ...
    @Query("SELECT p FROM SurchargeDiscount p WHERE LOWER(p.discDesc) LIKE LOWER(concat('%', :discDesc, '%')) ORDER BY p.discDesc ASC")
    List<SurchargeDiscount> findByDiscDescContainingIgnoreCaseOrderByDiscDesc(String discDesc);

    // auto generated by JPA, no need for code
    @Query("SELECT p FROM SurchargeDiscount p WHERE LOWER(p.isCategory) LIKE LOWER(concat('%', :isCategory, '%')) ORDER BY p.discDesc ASC")
    List<SurchargeDiscount> findByIsCategoryContainingIgnoreCaseOrderByDiscDesc(String isCategory);
    ...
    @Query("SELECT p FROM SurchargeDiscount p WHERE " +
        ...
        "ORDER BY p.discDesc ASC")
    List<SurchargeDiscount> findChildSurchargeDiscounts(@Param("parentDiscId") Integer parentDiscId);    

    // Auto-generated by JPA, no need for code
    @Query("SELECT p FROM SurchargeDiscount p ORDER BY p.discDesc ASC")
    List<SurchargeDiscount> findAllByOrderByDiscDesc();        
}
