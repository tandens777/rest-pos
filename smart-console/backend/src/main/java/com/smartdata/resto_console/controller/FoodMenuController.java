...
public class FoodMenuController {
    ...
    public void addItem(
        ...
        @RequestParam String lastupduserid) {
            ...
            itemService.addItem(item_code, item_desc, short_nm, sort_order, chinese_item_desc, 1, parent_cat_id, is_category,
    ...
    public void updateItem(
            ...
            @RequestParam String lastupduserid) {
                ...
                itemService.updateItem(id, item_code, item_desc, short_nm, sort_order, chinese_item_desc, 1, parent_cat_id, is_category,
                        station_id, per100g_flag, default_price, addon_price, picture_src, default_unit_code, disc_exempt,
                        allow_sc_on_exempt, non_vat_flag, active_flag, show_on_pos_flag, reorder_limit, track_invty_flag,
                        send_to_printer_flag, allow_dinein_flag, allow_pickup_flag, allow_delivery_flag, soldout_flag, lastupduserid);
    }
    ...
    public ResponseEntity<List<Item>> getItems(@RequestParam(required = false) String search) throws GenericNotFoundException {
        return ResponseEntity.ok(itemService.getItems(1, search));
    }

    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/all_food_menus")
    public ResponseEntity<List<Item>> getItemsOnly(@RequestParam(required = false) String search) throws GenericNotFoundException {
        return ResponseEntity.ok(itemService.getItemsOnly(1, search));
    }
