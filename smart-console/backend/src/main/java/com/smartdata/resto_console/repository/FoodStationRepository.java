...
public interface FoodStationRepository extends JpaRepository<FoodStation, Integer> {
    ...
    @Query("SELECT f FROM FoodStation f WHERE LOWER(f.stationNm) LIKE LOWER(concat('%', :searchTerm, '%')) ORDER BY f.stationId")
    List<FoodStation> findByNameContainingIgnoreCaseOrderById(@Param("searchTerm") String searchTerm);

    // Sort all by unit_code
    // auto generated by JPA, no need for code
    List<FoodStation> findAllByOrderByStationId();    
}
