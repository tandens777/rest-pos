...
BEGIN
  ...
  INSERT INTO pay_method (
      ...
      need_ref, need_expdt, short_nm, active_flag, bank_charges, sm_pay_type
  ) 
  VALUES (
      max_no, p_pay_mtd_desc, p_parent_pay_mtd_id, p_is_category, p_picture_src, 
      p_need_ref, p_need_expdt, p_short_nm, p_active_flag, p_bank_charges, p_sm_pay_type
  );
END;
$BODY$;
ALTER PROCEDURE public.insert_pay_method(
    character varying, int, character varying, character varying, character varying, 
    character varying, character varying, character varying, double precision, int
) OWNER TO postgres;
